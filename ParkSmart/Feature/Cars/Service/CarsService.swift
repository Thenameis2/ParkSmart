...
final class CarsServiceImpl: CarsService {
    ...
    func getAllCars() async throws -> [Car] {
            do {
                let snapshot = try await db.collection(carsPath).getDocuments()
                
                let cars = try await withThrowingTaskGroup(of: Car.self) { group in
                    var allCars: [Car] = []
                    
                    for document in snapshot.documents {
                        group.addTask {
                            let carData = document.data()
                            let groupId = carData[CarKeys.group.rawValue] as? String ?? ""
                            
                            // Fetch group name
                            let groupDoc = try await self.db.collection(self.groupsPath).document(groupId).getDocument()
                            let groupName = groupDoc.data()?["name"] as? String ?? ""
...
//    }
    ...
    func updateCarLocation(_ car: Car, location: CLLocation) async throws -> GeoPoint {
        return try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<GeoPoint, Error>) in
            ...
            // update the location in Firestore without resetting user-related fields
            self.db.collection(self.carsPath).document(car.id).updateData([
                CarKeys.location.rawValue: geoPoint
                // Do not update `currentlyInUse`, `currentlyUsedById`, `currentlyUsedByFullName`
            ]) { error in
                if let error = error {
                    continuation.resume(throwing: error)
                } else {
